...
const SellPage = () => {
  ...
  const integerRegex = /^\d*$/; // Integers only
  ...
  const handleInputChange = (e) => {
    ...
    // Only update formData if input is valid or empty (allow clearing)
    if (error === '' || value === '') {
  ...
  const handleImageChange = (e, index) => {
    ...
    if (files.length > 0) {
      ...
      // Clear image error if first image is uploaded
      if (index === 0) {
